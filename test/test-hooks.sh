...
set -x
http -a hook:express :3000/hooks path=/random hook='res.send(""+Math.random());'
http -a hook:express :3000/random
http -a hook:express :3000/hooks path=/echo method=post hook='res.send(req.body);'
http -a hook:express :3000/echo foo=bar
http -a hook:express :3000/hooks path=/count hook='res.send(""+res.locals.context.requestCount);'
http -a hook:express :3000/hooks path=/time hook='res.send(new Date());'
